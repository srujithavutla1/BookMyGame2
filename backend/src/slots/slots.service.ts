...
import { Model } from 'mongoose';
import { Slot } from './schemas/slot.schema';
import { Invitation } from '../invitations/schemas/invitation.schema'; // Add this import
import { SlotDto } from './dtos/slot.dto';
import { SlotPeopleAddedDto } from './dtos/SlotPeopleAdded.dto';
import { EventEmitter2 } from '@nestjs/event-emitter';
import { SlotStatusDto } from './dtos/slotStatus.dto';
...
export class SlotsService {
  ...
  constructor(
    @InjectModel(Slot.name) private slotModel: Model<Slot>,
    @InjectModel(Invitation.name) private invitationModel: Model<Invitation>, // Add this injection
    private readonly eventEmitter: EventEmitter2,
  ) {}
  ...
  async getAllUserSlots(email: string): Promise<Slot[]> {
    ...
    // Replace invitationsService call with direct invitationModel query
